-- Run once in Supabase → SQL Editor (after add-billing-details.sql)
-- International billing: structured address + ISO country code
...
drop function if exists public.submit_organization_plan_request(text, text, text, int, text, text, text, text, text);
drop function if exists public.submit_organization_plan_request(text, text, text, int, text, text, text, text, text, text, text, text);
...
declare
  ...
  v_tax_id text := upper(regexp_replace(coalesce(trim(p_tax_id), ''), '\s+', '', 'g'));
  v_street text := trim(p_billing_street);
  v_line2 text := nullif(trim(p_billing_address_line2), '');
  v_postal text := trim(p_billing_postal_code);
  v_city text := trim(p_billing_city);
  v_region text := nullif(trim(p_billing_region), '');
  v_country text := upper(trim(p_billing_country));
  v_address text;
begin
  ...
  if v_tax_id is null or length(v_tax_id) < 3 then
    return jsonb_build_object('ok', false, 'error', 'Enter a valid tax ID or VAT number.');
  end if;

  if length(v_street) < 3 then
    return jsonb_build_object('ok', false, 'error', 'Enter address line 1.');
  end if;

  if length(v_postal) < 2 then
    return jsonb_build_object('ok', false, 'error', 'Enter a valid postal or ZIP code.');
  end if;

  if length(v_city) < 2 then
    return jsonb_build_object('ok', false, 'error', 'Enter your city.');
  end if;

  if v_country is null or length(v_country) <> 2 then
    return jsonb_build_object('ok', false, 'error', 'Select your billing country.');
  end if;
  ...
  v_address := v_street
    || case when v_line2 is not null then ', ' || v_line2 else '' end
    || ', ' || v_postal || ' ' || v_city
    || case when v_region is not null then ', ' || v_region else '' end
    || ', ' || v_country;
  ...
  set
    ...
    billing_street = v_street,
    billing_address_line2 = v_line2,
    billing_postal_code = v_postal,
    billing_city = v_city,
    billing_region = v_region,
...
grant execute on function public.submit_organization_plan_request(text, text, text, int, text, text, text, text, text, text, text, text, text, text) to anon, authenticated;
...
begin
  ...
  return jsonb_build_object(
    ...
    'request', jsonb_build_object(
      ...
      'billing_street', v_row.billing_street,
      'billing_address_line2', v_row.billing_address_line2,
      'billing_postal_code', v_row.billing_postal_code,
      'billing_city', v_row.billing_city,
      'billing_region', v_row.billing_region,
      'billing_country', v_row.billing_country,
    ...
    'requests', (
      select coalesce(jsonb_agg(
        jsonb_build_object(
          ...
          'billing_region', r.billing_region,
